...
const LINKING_ERROR =
  `The package 'react-native-alipay-all' doesn't seem to be linked. Make sure: \n\n` +
...
const AlipayAll = NativeModules.AlipayAll
  ? NativeModules.AlipayAll
  : new Proxy(
      {},
      {
        get() {
...
export function alipay(payInfo: string): Promise<OrderResult> {
  return AlipayAll.alipay(payInfo);
}
...
export function authInfo(authInfoStr: string): Promise<AuthResult> {
  return AlipayAll.authInfo(authInfoStr);
}
...
export function setAlipayScheme(scheme: string): void {
  AlipayAll.setAlipayScheme(scheme);
}
/**
 * 设置支付宝沙箱环境，仅 Android
 * @param isSandBox
...
export function setAlipaySandbox(isSandbox: boolean): void {
  AlipayAll.setAlipaySandbox(isSandbox);
}
